...
TITLE_COLUMN    = "Title"                  # column header with the product title
...
MAX_WORKERS     = 4                        # adjust to your CPU/RAM
...
def sanitize_filename(name: str) -> str:
    """Remove illegal filesystem chars and replace spaces with underscores."""
    cleaned = re.sub(r'[\\/*?:"<>|]', "", name)
    return cleaned.strip().replace(" ", "_")

def download_row(session, csv_name, row_num, title, raw_urls):
    """
    Download all images for one row into:
      downloaded_images/<csv_name>/<sanitized_title>_row<row_num>/
    Returns: (row_num, semicolon-joined absolute paths).
    """
    safe_title = sanitize_filename(title) or f"row{row_num}"
    folder_name = f"{safe_title}_row{row_num}"
    download_dir = os.path.join(DOWNLOAD_ROOT, csv_name, folder_name)
...
for csv_path in glob.glob(os.path.join(CSV_DIR, "*.csv")):
    ...
    # prepare list of rows still to do: (csv_name, row_num, title, raw_urls)
    to_do = [
        (csv_name, idx+1,
         str(df.at[idx, TITLE_COLUMN] or ""),
         str(df.at[idx, URL_COLUMN]   or ""))
        for idx in df.index
        if not str(df.at[idx, "images_path"]).strip()
    ]

    # process in batches to re-init session periodically
    for batch_start in range(0, len(to_do), REINIT_EVERY):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {
                executor.submit(download_row, session, name, rn, title, urls): (rn, title)
